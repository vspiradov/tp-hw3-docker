...
if [ "$1" = "build_generator" ]; then
    ...
    echo "Запуск сервера Python на порту 8080..."
    docker run --rm -d -p 8080:8000 -v $(pwd)/data:/data -w /data python:3.12-slim python -m http.server 8000
    echo "Сервер поднят! Сделайте порт 8080 публичным в Codespaces."
